...
## Gemini eligibility analysis

Eligibility decisions are generated by Gemini from the tender and the submitted company profile. Each result includes a score, recommendation, reasoning, requirement-level evidence, and risks. TenderMind deliberately does **not** replace an unavailable Gemini response with hard-coded eligibility rules; those results are labelled `analysis_unavailable` instead.

Create a `.env` file before running the API or demo:
...
Run `npm test` to validate the Gemini assessment contract without making an external API call.
Run `npm run verify:gemini` to verify the configured key against the Gemini API.
